...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 22
        "*** REPLACE THIS LINE ***"
        return complete_eval(Thunk(complete_eval(self.apply(operands, env)), env))
        # END Problem 22
...
def do_and_form(expressions, env):
    ...
    while expressions:
        tail = (expressions.second == nil)
        and_expression = scheme_eval(expressions.first, env, tail)
        if scheme_falsep(and_expression) or and_expression is None:
            return False
        expressions=expressions.second
...
def do_or_form(expressions, env):
    ...
    while expressions:
        tail = (expressions.second == nil)
        or_expression = scheme_eval(expressions.first, env, tail)
        if scheme_truep(or_expression) and or_expression != None:
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(expressions.first.first):
        check_formals(expressions.first.second)
        body = expressions.second
        macro = MacroProcedure(expressions.first.second, body, env)
        env.define(expressions.first.first, macro)
...
def scheme_call_cc(function, env):
    ...
    continued = ContinuationProcedure(ContinuationError)
    child_frame = function.env.make_child_frame(function.formals, Pair(continued, nil))
    try:
        result = eval_all(function.body, child_frame)
...
class MuProcedure(UserDefinedProcedure):
    ...
    "*** REPLACE THIS LINE ***"
    def make_call_frame(self, args, env):
        """Creates a frame containing bindings from a let expression."""
        if isinstance(self, MuProcedure):
